...
def process_myth_file(filepath, motif_to_cluster, cluster_descriptions, motif_labels):
    ...
    residual_motif_descriptions = {
        mid: motif_labels.get(mid, "(label not found)") for mid in residual_motifs
    }
...
def main():
    ...
    motif_labels = fetch_all_a_motif_labels()
    ...
    for filename in os.listdir(MYTH_INPUT_DIR):
        if filename.endswith(".json") and filename not in [
            ...
            signature = process_myth_file(filepath, motif_to_cluster, cluster_descriptions, motif_labels)
