...
def load_clusters():
    all_clusters = []
    for filename in os.listdir(MYTH_OUTPUT_DIR):
        if filename.endswith(".json") and filename != "candidate_cluster_groups.json":
            try:
                with open(os.path.join(MYTH_OUTPUT_DIR, filename), "r", encoding="utf-8") as f:
                    data = json.load(f)
...
def group_clusters(clusters):
    ...
    # Convert member dicts to lists and sets to lists for JSON serialization
    for group in grouped:
        group["members"] = list(group["members"].values())
        for member in group["members"]:
            member["motifs"] = list(member["motifs"])
