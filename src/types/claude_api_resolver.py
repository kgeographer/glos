        ...
        for _, row in results_df.iterrows():
            ...
            try:
                ...
                if coords is not None:
                    ...
                    elif hasattr(coords, '__iter__') and not isinstance(coords, str):
...
def main():
    ...
    # TESTING: Limit to first 50 terms for initial run
    original_count = len(terms_df)
    # terms_df = terms_df.head(50)
    # resolver.log_print(f"TESTING MODE: Limited to first {len(terms_df)} terms (out of {original_count} total)")
    resolver.log_print(f"FULL DATASET MODE: {original_count} total terms to resolve")

    # Show the terms we'll be testing
    resolver.log_print(f"\nTerms to resolve (top 50 by usage):")
    for i, row in terms_df.head(20).iterrows():
        resolver.log_print(f"  {i+1:2}. '{row['ref_term']}' ({row['usage_count']} uses)")
    if len(terms_df) > 20:
        resolver.log_print(f"  ... and {len(terms_df) - 20} more")
    
    # Resolve with Claude API
    resolver.log_print(f"\nStep 2: Resolving {len(terms_df)} terms with Claude API...")
    results_df = resolver.resolve_all_terms(terms_df, batch_size=50, delay=1.5)
