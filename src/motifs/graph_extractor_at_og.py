...
def process_all_notes(input_directory, output_directory):
    ...
    with open(node_output_file, 'w') as node_file, open(edge_output_file, 'w') as edge_file, open(log_file_path, 'w') as log_file:
        for filename in os.listdir(input_directory):
            if filename.endswith('_piped_1line.txt'):  # Adjusting to .txt extension
